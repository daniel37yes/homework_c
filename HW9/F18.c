...
int find_max_in_row(int row[SIZE]) {
    int max = row[0];
...
// Функция для вычисления суммы максимумов строк
int sum_of_row_maximums(int matrix[SIZE][SIZE]) {
    int sum = 0;
    for (int i = 0; i < SIZE; i++) {
        sum += find_max_in_row(matrix[i]);
    }
    return sum;
}
...
int main() {
    ...
    int result = sum_of_row_maximums(matrix);
    printf("%d\n", result);
